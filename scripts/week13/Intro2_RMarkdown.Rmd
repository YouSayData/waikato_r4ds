...
You can write R code inline to calculate variables. For instance: We have data about `r comma(nrow(diamonds))` diamonds. Only `r nrow(diamonds) - nrow(smaller)` are larger than 2.5 carats.
...
Those are the counts for the different cuts:
...
And those are the counts for the different clarities:
